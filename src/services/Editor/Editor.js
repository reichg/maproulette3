...
export const editTask = function(editor, task, mapBounds, options, taskBundle) {
  return function(dispatch) {
    if (isWebEditor(editor)) {
      ...
      else if (AsSuggestedFix(task).isVersion2()) {
        // Suggested fix with XML change
        openJOSM(
          dispatch, editor, task, mapBounds, josmLoadAndZoomURI, taskBundle, {
            layerName: editor === JOSM_LAYER ?
                       ...
                       `MR Task ${task.id} Proposed Changes` // layers will be merged
          }
        ).then(() => setTimeout(() =>
          sendJOSMCommand(
...
export const josmImportURI = function(dispatch, editor, task, mapBounds, taskBundle, options) {
  return josmHost() + 'import?' + [
    ...
    `layer_name=${encodeURIComponent("MR Task " + task.id + " Proposed Changes")}`,
    `url=${process.env.REACT_APP_MAP_ROULETTE_SERVER_URL}/api/v2/task/${task.id}/sfxml`
  ].join('&')
}
